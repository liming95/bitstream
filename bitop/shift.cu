...
#define SET_HIGH_BIT(number, size) (number | (1 << (size-1)))
#define CLEAR_HIGH_BIT(number, size) (number ^ (1 << (size-1)))
#define CLEAR_LOW_BIT(number) (number ^ 1)
#define WAIT(number, size, condition) while((__ldg(&number) >> (size-1)) != condition)
#define grid_size 1
#define block_size 2
...
__global__ void shift_left(uint32_t *bit_stream, int size, int shift_count, uint32_t* shift_global){
    ...
    if(threadIdx.x < blockDim.x-1){
        ...
        shift_global[block_idx] |= bit_stream[thread_idx] >> (size - shift_count);
        __threadfence();
    ...
    if(threadIdx.x == 0 && blockIdx.x != 0){
        ...
        shift_block[0] = shift_block[blockIdx.x];
        shift_block[blockIdx.x] = CLEAR_HIGH_BIT(shift_block[blockIdx.x], sizeof(uint32_t));
    }
